...
$project = Join-Path $repo 'scratch\npu-reset\current-rv32-concurrent-final'
$out = Join-Path $repo 'research\checkpoints\2026-09-05-npu-reset\ghidra-current-rv32-concurrent-final'
...
if ((Get-FileHash -LiteralPath $input).Hash.ToLowerInvariant() -ne 'e743d1b59a9ca6d043e38ff71075e8d28702a104b94abb17a4035514efda4643' -or
    ...
    throw 'Firmware version changed; revisit concurrency annotations'
}
...
$arguments = @($project,'CurrentNpu','-max-cpu','2','-analysisTimeoutPerFile','1800',
    ...
    '-preScript','ModelNpuConcurrency.java',
...
'PASS: current NPU full auto-analysis with shared SRAM and permanent workers'
