...
$project = Join-Path $repo 'scratch\npu-barrier\current-rv32-irq'
$out = Join-Path $repo 'research\checkpoints\2026-09-05-npu-barrier\ghidra-irq'
...
if ($text -match 'decompiled=false' -or $text -notmatch 'FUNCTION analysis_uart_irq_840047a0' -or
...
'PASS: UART IRQ seeded; current NPU auto-analysis and export complete'
