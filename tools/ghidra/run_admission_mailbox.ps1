...
$out = Join-Path $repo 'research\checkpoints\2026-09-05-npu-admission\ghidra-mailbox'
...
if ((Get-FileHash -LiteralPath $input).Hash.ToLowerInvariant() -ne 'e743d1b59a9ca6d043e38ff71075e8d28702a104b94abb17a4035514efda4643' -or
    (Get-FileHash -LiteralPath $data).Hash.ToLowerInvariant() -ne '61a75afb052feed2ceb2f3023e16f50317c05c78f9c8e564bf01924ce39c7ec1') {
...
$arguments = @($project,'CurrentNpu','-max-cpu','2','-analysisTimeoutPerFile','1800',
    ...
    '-preScript','ModelNpuConcurrency.java','-preScript','SeedNpuUartIrq.java',
    '-preScript','SeedNpuMailboxCallbacks.java',
...
if ($text -match 'decompiled=false' -or $text -notmatch 'FUNCTION analysis_mailbox_wifi_84003a9c' -or
    $text -notmatch 'FUNCTION analysis_mailbox_tunnel_84003a86' -or
    $text -notmatch 'failed_decompilations=0') { throw 'Incomplete firmware export' }
if ((Get-Content -Raw -LiteralPath (Join-Path $out 'analysis.log')) -match 'Analysis timed out') { throw 'Analysis timeout' }
'PASS: mailbox callbacks seeded; NPU auto-analysis and export complete'
