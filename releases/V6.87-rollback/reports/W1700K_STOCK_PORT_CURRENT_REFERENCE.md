...
Last updated: 2026-09-01 20:40 +03:00
...
- Stock timeout literals are poll counts over 100-microsecond delays: WLAN
  stop SET is 6 seconds and ownership GET is 100 milliseconds. The corrected
  constants are compiled into the candidate; no wrong-timeout image exists.
- PCI shutdown now uses a non-destructive quiesce path. True unregister waits
  indefinitely for NPU ownership value zero before freeing RRO/token/DMA/NPU
  state, matching the recovered stock safety boundary.
